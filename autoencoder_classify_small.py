...
class Classifier(nn.Module):
    def __init__(self, encoder, enc_code_dim=4):
        super().__init__()
...
encoder.load_state_dict(torch.load(f'encoder_bottleneck_{encoded_space_dim}.pth'))
encoder.to(device)

classifier = Classifier(encoder, enc_code_dim=encoded_space_dim)
params_to_optimize = [
...
# save the model
torch.save(classifier.state_dict(), f'classifier_small_num_features_{encoded_space_dim}_num_epochs{num_epochs}.pth')